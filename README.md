...
- ✅ **URL 构建器** - 可视化片段编辑，支持环境变量注入，支持整行文本域编辑长路径
- ✅ **URL 片段着色** - 编辑区与预览区按片段位置循环 8 色着色，变量以粗体+蓝色圆角边框区分
...
### 2026-06-29（URL 编辑增强 + 片段着色）
- **整行 URL 编辑** — 预览区右上角新增 ✏️ 按钮，点击展开 auto-grow textarea 编辑完整模板路径（保留 `{var}` 语法），Enter 确认 / Escape 取消，确认后自动解析回分段
- **片段 8 色着色** — 编辑区与预览区按片段位置循环 8 种颜色背景，变量以粗体 + 蓝色圆角边框区分，文字色与普通片段一致
- **预览区重构** — 去掉"完整路径"标签行，操作按钮（📋/✏️ 或 ✅/❌）固定在右上角叠层，路径内容多行自动折行
- **主题适配** — Dark 主题使用 sat 45% / L 28% 深色版本，Light 主题使用 sat 35% / L 80% 浅色版本，色相均匀分布 45° 一格
